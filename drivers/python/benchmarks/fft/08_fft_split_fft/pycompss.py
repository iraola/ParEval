# Driver for 08_fft_split_fft
# """ Compute the fourier tansform and return two separate arrays of real and imaginary parts.
# """
...
def validate(ctx: Context):
    ...
    for _ in range(max_attempts):
        ...
        seq_res_real, seq_res_imag = correct_main(ctx.x)

        # Parallel execution
        par_res_real, par_res_imag = main(ctx.x)

        # Check equality
        if not fequal(par_res_real, seq_res_real, eps=1e-6) or not fequal(par_res_imag, seq_res_imag, eps=1e-6):
            return False
